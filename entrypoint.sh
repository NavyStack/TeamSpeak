...
if [ "$1" = 'ts3server' ]; then
  ...
  if [ "$(id -u)" = '0' ]; then
    ...
    if [ -n "$UID" ] && [ -n "$GID" ]; then
      ...
      usermod -u "$UID" ts3server
      groupmod -g "$GID" ts3server
    else
      echo "Using default UID = 1001 / GID = 1001"
    fi
    chown -R ts3server:ts3server /var/ts3server /var/run/ts3server /opt/ts3server
